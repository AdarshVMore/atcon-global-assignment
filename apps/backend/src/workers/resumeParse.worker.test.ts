...
import { createResumeParseProcessor } from "./resumeParse.worker.ts";
import type { CandidateRepository } from "../candidates/candidate.repository.ts";
import type { ResumeInformationExtractor } from "../candidates/resumeInformationExtractor.ts";
import type { ResumeRepository, UpdateResumeStatusInput } from "../candidates/resume.repository.ts";
import type { ResumeStorage } from "../shared/storage/resumeStorage.ts";

const PDF_FIXTURE_PATH = new URL("../candidates/__fixtures__/sample-resume.pdf", import.meta.url).pathname;
